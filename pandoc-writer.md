...
Debianでは、aptを使ってインストールできるので、インストールしておきましょう。
...
Pandoc参照用odtファイルは、スタイルを設定しただけの空のodtファイルですが、独自に作成したodtファイルを使うとうまく変換できない場合もあるので、オリジナルのreference.odtをひな型にして作成します。

reference.odtは、Debianでは/usr/share/pandoc/data/reference.odtにあります。
これをコピーするか、マニュアルにあるようにようにして保存したものを用意します。

    $ pandoc --print-default-data-file reference.odt > reference.odt

編集は、Writerでreference.odtを開いてスタイルを設定しますが、言語設定が日本語環境向けではないので、はじめに言語設定を修正しておきます。
...
このほかに設定で注意する点としては、標準ページスタイルの用紙サイズが「レター」になっていることと余白の幅が変更されています。

標準設定では用紙サイズ「A4」、余白「2.0cm」ですが、出力する文書によってサイズ、余白は変わるので、メニューバー[書式]-[ページ]から使用する用紙サイズと余白に設定しておきます。

### スタイルを作成する

ここからはスタイルを作ります。
スタイルについては、手を入れる個所が多く、文章での説明は長くなるので、用意したサンプルファイル(reference-japanese.odt)を元にポイントを説明します。
...
最初に「規定のフォント」を戻しましたが、標準では欧文(西欧諸言語)と和文(アジア諸言語)それぞれ違うフォント、違うサイズで設定されているので、見栄えを整えるときに思わぬところで悩む原因になります。

和文と欧文をそれぞれ違うフォント設定にしたい場合は、スタイルを利用して設定すると変更した個所が明確になりトラブルが少なくなるので、オプションの「規定のフォント」は和文フォントに揃えておくとよいでしょう。
...
LibreOfficeのスタイル作成の基本として、段落スタイルの「標準」はすべての段落スタイルの大元になるので、なにはなくとも最初に設定します。
とはいえ、作り込み過ぎると継承するスタイルにも影響が出るので、設定は基本的な設定だけにとどめておきましょう。
...
大見出しである「見出し1」は、体裁タブにある区切り「挿入」にチェックを入れて、種類を「ページ」にしておくと、「見出し1」が出るとページの頭から始まるようになるので設定しておくとよいでしょう。
...
変換の前に、毎回、ネットワークからダウンロードするのは時間もかかるので、一度ローカルに保存してから変換をします。

HTMLの保存は、Firefox/Iceweaselの「ページを保存」を使うと手軽に画像を含めた完全な形で保存できるのでお薦めです。それ以外でも画像なども含めて保存できるなら、どの方法を使っても構いません。

では、Pandocで変換してみましょう。最初と違うのは「--reference-odt=reference-japanese.odt」で作成した参照用odtを指定している部分が違います。

    $ pandoc -f html -t odt --reference-odt=reference-japanese.odt -o ideditor.odt ideditor.html
...
この機能を使うと簡単に冊子用の印刷ができるので、A5サイズの冊子として編集していきます。

「パンフレットの印刷」をするには、ページを印刷する用紙(A4)の半分のサイズ(A5)にする必要があるので用紙サイズを変更します。

まず、サイドバーのプロパティから標準ページスタイルの用紙サイズを「A5」、余白を「狭い」(1.27cm)に変更しました。そして、表紙もつけたいのでページスタイルの「最初のページ」も同じ設定にしました。
これでA4に2ページ分、面付けできるようになりました。

あとは用紙に収まるように編集していきますが、画像については注意が必要です。
Pandocは画像のアンカーをすべて「文字として」に設定して変換するので、画像を移動させようとしても移動することはできません。

画像を移動するには、画像を切り取って移動先で貼り付けをする。
移動だけでなくレイアウトも同時に変更する場合は、アンカーを「段落に」に変更して移動と調整をする必要があります。


編集した部分を説明すると、画像の配置を変更した以外はアイコンの機能説明が箇条書きで書いてあり紙としては収まりが悪いので表組みに変更しましたが、表組は簡単に作成できます。

1. 箇条書きを解除
2. 文章と画像の間にタブを挿入
3. 表組みしたい部分を選択して、[表]ボタンを押す





編集ができたら「パンフレットの印刷」で出力します。

「パンフレットの印刷」で出力するには、メニューバー[ファイル]-[印刷]で印刷ダイアログを開き、
「ページレイアウト」タブにあるラジオボタン「パンフレット」をクリックします。

印刷する場合は、このままプリンターを選んで両面印刷をするとパンフレットとして印刷できます。

PDFに出力するには、「オプション」タブにある「ファイルへ出力」にチェックを入れると「印刷」ボタンが「ファイルへ出力」ボタンに変わるので、PDFとして保存できます。

PDFに出力するとコンビニなどで印刷もできるので、

## 最後に

例では、HTMLで書かれた文書をodtに変換して編集しましたが、Pandocはいろいろな文書フォーマットに対応しているので、長文の原稿をMarkdownで書いて、
